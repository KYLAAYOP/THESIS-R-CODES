...
data <- read.csv("vancouver_crime.csv", header = TRUE)
head(data)
tail(data)
...
mean_TfA <- mean(data$TfA, na.rm = TRUE)
data$centered_TfA <- data$TfA - mean_TfA

#Standardizing the Data
#To standardize the data (subtract the mean and divide by the standard deviation):Compute the standard deviation
sd_TfA <- sd(data$TfA, na.rm = TRUE)
data$standardized_TfA <- (data$TfA - mean_TfA) / sd_TfA
head(data)

ts_data <- ts(data$centered_TfA, frequency = 12, start = c(min(data$Year), min(data$Month)))

# Plot time series
ggplot(data, aes(x = Time, y = centered_TfA)) +
  geom_line() +
  labs(title = "TfAs Over Time", x = "Time", y = "centered_TfA") +
  theme_minimal()

# Plot time series
ggplot(data, aes(x = Time, y = standardized_TfA)) +
  geom_line() +
  labs(title = "TfAs Over Time", x = "Time", y = "standardized_CO2") +
  theme_minimal()

# Plot time series
ggplot(data, aes(x = Time, y = TfA)) +
  geom_line() +
  labs(title = "TfAs Over Time", x = "Time", y = "TfA") +
  theme_minimal()

#' 
#' Evaluate Different Lag Orders
#' PACF Plot and BIC for GP-NAR
## --------------------------------------------------------------
...
ggplot(data.frame(Time = 1:length(Y_test), Actual = Y_test, Predicted = y_pred), aes(x = Time)) +
  ...
  theme_minimal()+
  labs(title = "GP-NAR Prediction", y = "Theft from Auto Incident", x = "Time",color = "Legend")
...
ggplot(data_plot, aes(x = Time)) +
  ...
  geom_line(aes(y = Predicted, color = "Predicted")) +
  theme_minimal() +
  labs(title = "GP-NAR Forecast with Kernel-Based Uncertainty", y = "Theft from Auto Incident", x = "Time",color = "Legend" )
