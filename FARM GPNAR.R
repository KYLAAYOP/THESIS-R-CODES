...
data <- read.csv("farmgate_prices.csv", header = TRUE)
head(data)
summary(data)
...
#Centering the Data
#To center the data (subtract the mean):Compute the mean of Farmgate Price concentration
mean_Price <- mean(data$Price, na.rm = TRUE)
data$centered_Price <- data$Price - mean_Price
...
data$standardized_Price <- (data$Price - mean_Price) / sd_Price
head(data)

ts_data <- ts(data$centered_Price, frequency = 12, start = c(min(data$Year), min(data$Month)))

# Plot time series
ggplot(data, aes(x = Time, y = centered_Price)) +
  geom_line() +
  labs(title = "Farmgate Prices Over Time", x = "Time", y = "centered_Price") +
  theme_minimal()

# Plot time series
ggplot(data, aes(x = Time, y = Price)) +
  geom_line() +
  labs(title = "Farmgate Prices Over Time", x = "Time", y = "Price") +
  theme_minimal()
...
ggplot(data.frame(Time = 1:length(Y_test), Actual = Y_test, Predicted = y_pred), aes(x = Time)) +
  ...
  theme_minimal()+
  labs(title = "GP-NAR Prediction", y = "Farmgate Price", x = "Time",color = "Legend")
...
ggplot(data_plot, aes(x = Time)) +
  ...
  geom_line(aes(y = Predicted, color = "Predicted")) +
  theme_minimal() +
  labs(title = "GP-NAR Forecast with Kernel-Based Uncertainty", y = "Farmgate Price", x = "Time",color = "Legend")
