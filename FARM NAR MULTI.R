...
ts_data <- ts(data$Price, frequency = 12, start = c(min(data$Year), min(data$Month)))
...
# Plot time series
ggplot(data, aes(x = Time, y = standardized_Price)) +
  geom_line() +
  labs(title = "Farmgate Prices Over Time", x = "Time", y = "standardized_Price") +
  theme_minimal()
